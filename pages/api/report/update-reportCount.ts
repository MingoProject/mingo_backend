...
import { updateReportUserCount } from "@/lib/actions/report.action"; // Import logic cập nhật báo cáo
import type { NextApiRequest, NextApiResponse } from "next";
import corsMiddleware, {
  authenticateToken,
} from "@/middleware/auth-middleware"; // Middleware xác thực token người dùng
  ...
  corsMiddleware(req, res, async () => {
    if (req.method === "PATCH") {
      try {
        ...
        // Gọi action để cập nhật trạng thái báo cáo
        const updatedReport = await updateReportUserCount(reportId);

        return res.status(200).json(updatedReport);
      } catch (error) {
        console.error("Error updating report user count:", error);
